    ...
    <div class="card card-custom mb-4 border-0 shadow-sm">
      <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-3">
        ...
        <div class="d-flex align-items-center gap-3 flex-wrap">
          <div class="input-group input-group-merge shadow-sm custom-input-group date-filter-group"
            style="width: 210px;">
            <span class="input-group-text bg-white border-end-0 text-primary">
              <i class='bx bx-calendar'></i>
            </span>
            <input type="date" class="form-control border-start-0 ps-1 text-dark" v-model="filterDate" />
            <button v-if="filterDate" class="btn btn-outline-secondary border-0 btn-reset-date" @click="resetDate"
          ...
          <div class="input-group input-group-merge shadow-sm custom-input-group" style="width: 250px;">
            ...
            <input type="text" class="form-control border-start-0 ps-1 text-dark" placeholder="Tìm tên, mã HĐ..."
              v-model="searchQuery" />
          </div>

          <button class="btn btn-gradient-success shadow-md d-flex align-items-center fw-bold px-4 rounded-pill"
            @click="exportToExcel">
            ...
            <tr v-else v-for="invoice in paginatedInvoices" :key="invoice.maHD">
              ...
              <td>
                <div class="d-flex align-items-center">
                  <div class="avatar-wrapper me-2">
                    <img v-if="invoice.hinhAnhNhanVien" :src="getFullImageUrl(invoice.hinhAnhNhanVien)"
                      @error="handleImageError" class="avatar-img" />
                    <div v-else class="avatar-initial-circle">
                      <img :src="getDefaultAvatar()" class="avatar-img" />
                    </div>
                  </div>
                  <div class="d-flex flex-column">
              ...
              <td class="text-center">
                <div class="d-flex justify-content-center gap-2">
                  <button class="btn btn-icon btn-soft-primary shadow-sm" @click="openInvoiceDetail(invoice)"
                    title="Xem chi tiết">
      ...
      <div class="modal-dialog-custom">
        <div class="modal-content border-0 shadow-lg">
          ...
          <div class="modal-body p-0 bg-light-gray modal-body-scroll">
            <div id="invoice-print-area" class="invoice-paper shadow-sm mx-auto">
              ...
              <div class="d-flex justify-content-between align-items-start mb-4">
                <div>
                  <h3 class="fw-bold text-primary text-uppercase mb-1 brand-title" style="letter-spacing: 2px;">LUXURY
                    HOTEL</h3>
              ...
              <div class="row mb-4">
                <div class="col-6">
                  ...
                  <p class="text-dark small mb-0"><i class='bx bx-phone me-1'></i>{{ selectedInvoice?.sdt || '---' }}
                  </p>
                </div>
              ...
              <div class="mb-4" v-if="selectedInvoice?.chiTietPhong?.length">
                <h6
                  class="fw-bold text-dark small border-bottom border-2 border-primary pb-2 mb-3 d-flex align-items-center">
              ...
              <div class="mb-4" v-if="selectedInvoice?.dichVu?.length">
                <h6
                  class="fw-bold text-dark small border-bottom border-2 border-primary pb-2 mb-3 mt-4 d-flex align-items-center">
              ...
              <div v-if="(!selectedInvoice?.chiTietPhong?.length) && (!selectedInvoice?.dichVu?.length)"
                class="text-center py-4 bg-light rounded mt-3">
              ...
              <div class="row justify-content-end">
                <div class="col-md-6">
                  ...
                  <div class="d-flex justify-content-between mb-2 text-muted" v-if="calculatePhuThu(selectedInvoice) > 0">
                    <span>Tiền phòng & Dịch vụ:</span>
                    <span>{{ formatCurrency(selectedInvoice?.tongTienGoc - calculatePhuThu(selectedInvoice)) }}</span>
                  </div>

                  <div class="d-flex justify-content-between mb-2 text-dark" v-if="calculatePhuThu(selectedInvoice) > 0">
                    <span class="fw-medium">Phụ thu phát sinh:</span>
                    <span class="fw-bold text-warning">+ {{ formatCurrency(calculatePhuThu(selectedInvoice)) }}</span>
                  </div>

                  <div class="d-flex justify-content-between mb-2" :class="{'border-top pt-2 mt-2': calculatePhuThu(selectedInvoice) > 0}">
                    <span class="text-muted fw-bold">Tổng tiền gốc:</span>
                  ...
                  <div class="d-flex justify-content-between mb-2" v-if="selectedInvoice?.soTienGiam > 0">
                    <span class="text-danger">Giảm giá (Voucher):</span>
                    <span class="text-danger fw-bold">- {{ formatCurrency(selectedInvoice?.soTienGiam) }}</span>
                  </div>

                  <div class="d-flex justify-content-between mb-2" v-if="selectedInvoice?.tienCoc > 0">
                    <span class="text-success">Đã thanh toán trước (Cọc):</span>
                    <span class="text-success fw-bold">- {{ formatCurrency(selectedInvoice?.tienCoc) }}</span>
                  </div>

                  <div class="d-flex justify-content-between border-top border-dark pt-3 mt-2 align-items-center">
                    <span class="fw-bolder text-dark fs-5 text-uppercase">
                      <template v-if="(selectedInvoice?.tongTienGoc - selectedInvoice?.soTienGiam - selectedInvoice?.tienCoc) > 0">
                        Khách cần thanh toán:
                      </template>
                      <template v-else-if="(selectedInvoice?.tongTienGoc - selectedInvoice?.soTienGiam - selectedInvoice?.tienCoc) < 0">
                        Tiền thừa trả khách:
                      </template>
                      <template v-else>
                        Đã thanh toán đủ:
                    ...
                    <span class="fw-bolder fs-3 total-price" 
                          :class="{
                            'text-primary': (selectedInvoice?.tongTienGoc - selectedInvoice?.soTienGiam - selectedInvoice?.tienCoc) >= 0,
                            'text-success': (selectedInvoice?.tongTienGoc - selectedInvoice?.soTienGiam - selectedInvoice?.tienCoc) < 0
                          }">
                      {{ formatCurrency(Math.abs(selectedInvoice?.tongTienGoc - selectedInvoice?.soTienGiam - selectedInvoice?.tienCoc)) }}
                    </span>
                  </div>
          ...
          <div class="modal-footer px-4 py-3 bg-white border-top d-flex justify-content-end gap-2 rounded-bottom">
            ...
            <button type="button"
              class="btn btn-gradient-primary px-4 d-flex align-items-center shadow-md fw-bold rounded-pill"
              @click="printInvoice">
...
const calculatePhuThu = (invoice) => {
    if (!invoice) return 0;
    
    // 1. Tính tổng tiền phòng
    const tienPhong = invoice.chiTietPhong?.reduce((sum, item) => sum + (item.thanhTien || 0), 0) || 0;
    
    // 2. Tính tổng tiền dịch vụ
    const tienDichVu = invoice.dichVu?.reduce((sum, item) => sum + (item.thanhTien || 0), 0) || 0;
    
    // 3. Phụ thu = Tổng gốc - (Phòng + Dịch vụ)
    const phuThu = invoice.tongTienGoc - tienPhong - tienDichVu;
...
.btn-soft-dark {
  background: rgba(86, 106, 127, 0.15);
  color: #566a7f;
  border: none;
  border-radius: 50%;
  width: 38px;
...
.btn-reset-date:hover {
  color: #ff3e1d;
  /* Chuyển sang màu đỏ khi hover để báo hiệu xóa */
  background: transparent;
  transform: rotate(-90deg);
  /* Hiệu ứng xoay nhẹ khi hover cho sinh động */
}
...
@media print {
  ...
  @page {
    margin: 15mm;
    /* Tạo khoảng trống an toàn 4 cạnh giấy */
  }
  ...
  .modal-footer,
  .modal-footer button,
  /* Ẩn cụ thể các button trong footer */
  .btn-close-custom,
  .btn-icon,
  .card-custom,
  ...
  #invoice-print-area {
    ...
    /* Thêm padding nội bộ để nội dung lùi vào trong */
